...
class MyData_test(Dataset):
    def __init__(self, root_dir_SAR, root_dir_EO, transform=None):
        ...
        self.all_files = []
        # for file in os.listdir(self.root_dir_SAR):
    ...
    def __getitem__(self, index):
        ...
        name = int(img_path_SAR.split('/')[-1].split('_')[-1].split('.')[0])
        if self.transform:
            img_SAR = self.transform(img_SAR)
            img_EO = self.transform(img_EO)
        return img_SAR, img_EO, name
...
def test(model_path):
    ...
    F = torch.load('swinB_F_bestacc_Track2_EO_iteration_num2_fortest.pth')
    F = F.cuda()
    C = Classifier(10)
    C.apply(inplace_relu)
    C = torch.load('swinB_C_bestacc_Track2_EO_iteration_num2_fortest.pth')
    C = C.cuda()
    F.eval()
    C.eval()
    with torch.no_grad():
    ...
    sio.savemat('EO_iteration_num2_fortest_bestacc.mat', {'preds': all_pred_outputs.numpy()})
    # #################################################################################
    ...
    # ######################### write the .csv file ##########################
    csv_file = open('results_Track2_EO_iteration_num2_fortest_bestacc.csv', 'w', newline='')
